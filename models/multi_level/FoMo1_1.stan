...
transformed data {
  ...
  // normalise delta so that 1 = distance from
  // previously selected item to closest remaining
...
generated quantities {
  ...
  */
...

  {
    ...
    vector[n_targets] S_q, psi_q, delta_q;
    ...
    for (t in 1:n_trials) {
      ...
      for (ii in 1:n_targets) {
        ...
        if (ii > 1) 
        {
          ...
          // now scale delta
          delta_q = scale_prox(delta_q, remaining_items_q, n_targets);
        }
