...
functions {
  ...
  vector compute_spatial_weights(int n, int n_targets, 
    ...
    vector[n_targets] prox_weights, absdir_weights, reldir_weights;
...
parameters {
  ...
  */
  ...
  matrix[4*K, L] z_u; // 3*K as we have three params
  ...
  */
  ...
  for (k in 1:K) {
    for (l in 1:L) {
      ...
      u_log_theta[k, l, 1] = (log_theta[k, 1] + z_w[8*(k-1)+1, l] .* sigma_w[k]);
      u_log_theta[k, l, 2] = (log_theta[k, 2] + z_w[8*(k-1)+2, l] .* sigma_w[k]);
      u_log_theta[k, l, 3] = (log_theta[k, 3] + z_w[8*(k-1)+3, l] .* sigma_w[k]);
      u_log_theta[k, l, 4] = (log_theta[k, 4] + z_w[8*(k-1)+4, l] .* sigma_w[k]);
      u_log_theta[k, l, 5] = (log_theta[k, 5] + z_w[8*(k-1)+5, l] .* sigma_w[k]);
      u_log_theta[k, l, 6] = (log_theta[k, 6] + z_w[8*(k-1)+6, l] .* sigma_w[k]);
      u_log_theta[k, l, 7] = (log_theta[k, 7] + z_w[8*(k-1)+7, l] .* sigma_w[k]);
      u_log_theta[k, l, 8] = (log_theta[k, 8] + z_w[8*(k-1)+8, l] .* sigma_w[k]);
...
generated quantities {
  ...
  */
  //////////////////////////////////////////////////////////////////////////////
  {
    ...
    vector[n_targets] S_j, phi_j, delta_j, psi_j;
