/* ####### FoMo (Foraging Model 1.x)  #######

This is a modification of model 1.1 that 
adds in previous weight
...
model {
  ...
  for (ii in 1:N) {
    ...
    weights = weights .* spatial_weights + exp(b_m[kk]) * prev_weights;
    ...
    // now save prev_weights for use in next selection
