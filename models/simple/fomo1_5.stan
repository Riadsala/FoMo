...
FoMo V1.5 (single-level)

This model adds absolute direction (phi)
And removes relative direction (psi)
...
parameters {
  ...
  array[K] vector[3] log_theta; // mixing proportions for abs directions
}
...
model {
  ...
  for (ii in 1:N) {
    ...
    weights = compute_weights_v13(
      b_a[x], b_s[x], rho_delta[x], log_theta[x], kappa,
