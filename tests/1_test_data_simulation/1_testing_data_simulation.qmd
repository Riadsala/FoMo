...
stimuli_params <- list(n_item_class = 4,
                       n_item_per_class = c(12, 12, 10, 10), 
...
d <- sim_foraging_trial(trl = 1, stimuli_params, foraging_params, dev_output = TRUE)

plot_a_trial(d$stim, d$found, segLabel = "psi")

ggplot(d$found, aes(psi)) + geom_histogram(bins = 8)
```
...
d <- tibble(phi = seq(0, 2*pi, 0.01),
            z = compute_all_von_mises(phi=phi, 
                                      abs_dir_tuning$theta, 
                                      abs_dir_tuning$kappa))
