...
d_list <- prep_data_for_stan(d$found, d$stim, c("spatial", "item_class"))
d2_list <- add_priors_to_d_list(d_list, modelver = "1.2")

# run model
m2 <- mod$sample(data = d2_list, 
                 chains = 4, parallel_chains = 4, threads = 4,
                 refresh = 100, 
                 iter_warmup = iter, iter_sampling = iter,
                 sig_figs = 3)
...
# extract post
post2 <- extract_post(m2, d, multi_level = FALSE, absdir = FALSE)

# plot model
plot_model_fixed(post2, gt = list(b_a = plogis(item_class_weights[[1]]),
                                  b_stick = b_stick,
                                  rho_delta = rho_delta,
                                  rho_psi = rho_psi))
