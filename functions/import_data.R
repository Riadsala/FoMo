...
import_bhat2025 <- function(data_path = "../data/", small_test) {
    ...
    p_folders <- dir(paste0(data_path, "bhat2025"))
    p_folders <- p_folders[!str_detect(p_folders, ".txt")]
    ...
    for (pp in 1:length(p_folders)) {
      ...
      p_file_found <- dir(paste0(data_path, "bhat2025/", p_folders[pp]), "_found.csv", full.names = TRUE)
      p_file_stim <- dir(paste0(data_path, "bhat2025/", p_folders[pp]), "_stim.csv", full.names = TRUE)
      ...
      # age and gender
      filename <- str_split_i(p_file_found, "/", 7)
