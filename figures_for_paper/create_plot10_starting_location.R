...
plot_a_trial <- function(ds, df, 
  ...
  plt_human <- ggplot(ds, aes(x, y)) + 
    geom_path(data = df,colour = "grey80")  +
    geom_point(data = ds1, size = 5, aes(shape = factor(item_class)), color = "#B0E8E8") +
  ...
  plt_model <- ggplot(data = ds, aes(x, y)) + 
    geom_path(data = predf, 
                  aes(x, y),
                  alpha = 0.5, colour = "darkviolet") + 
    geom_point(data = ds1, size = 5, aes(shape = factor(item_class)), color = "#B0E8E8") +
    geom_point(data = ds2, size = 5, aes(shape = factor(item_class)), color = "#BE0032") +
...
ggsave("figs/fig10_examples.pdf", width = 15, height = 6)
