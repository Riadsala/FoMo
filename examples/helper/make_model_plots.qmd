...
- accuracy plots (including a plot that shows how accuracy changes across trials, an average accuracy plot which will show the current model version alongside any other model versions you have calculated for easy comparison, and a per-person accuracy plot comparing your chosen model against a baseline comparison)
- run statistics plots (observed vs. predicted) & correlations
- posterior density plots (including theta plots for relevant model versions, and random effects plots)
...
model_comp <- gsub("1", "1_", model_comp)
...
## Average accuracy (for all model versions)
