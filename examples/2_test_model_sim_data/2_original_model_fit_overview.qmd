...
We will fit model 1.2 (basically, the corrected original model)
...
```{r fitmodelsimdata1, cache=TRUE}
iter = 500
mod <- cmdstan_model("../../models/multi_level/FoMo1.stan", 
...
trl_stim <- filter(d$ds, person == pp, condition == 2, trial == trl)
trl_found <- filter(d$df, person == pp, condition == 2, trial == trl)
...
```{r}
rl <- get_run_info_over_trials(d$df) %>% 
