...
```{r}
datasets <- c("kristjansson2014plos")
```
...
# m <- readRDS("scratch/clarke2022qjep_train_1_0.model")
# post <- extract_post(m, d)
# plot_model_fixed(post)
```
...
d <- import_data("kristjansson2014plos")
m <- readRDS("scratch/kristjansson2014plos_train_1_1.model")
post <- extract_post(m, d)
plot_model_fixed(post)
```
...
# d <- import_data("clarke2022qjep")
# m <- readRDS("scratch/clarke2022qjep_train_1_1.model")
# post <- extract_post(m, d)
# plot_model_fixed(post)
```
...
# d <- import_data("tagu2022cog")
# m <- readRDS("scratch/tagu2022cog_train_1_1.model")
# post <- extract_post(m, d)
# plot_model_fixed(post)
```
...
d <- import_data("kristjansson2014plos")
m <- readRDS("scratch/kristjansson2014plos_train_1_2.model")
post <- extract_post(m, d)
plot_model_fixed(post)
```
...
```{r}
rl <- read_csv("scratch/run_statistics1_0.csv")
...
```{r}
rl <- read_csv("scratch/run_statistics1_1.csv")
...
```{r}
rl <- read_csv("scratch/run_statistics1_2.csv")
